...
if position_active:
    input_dim = 3
    writer = SummaryWriter("classify/tensorbd/" + object_cls.name + "/position_" + str(object_cls.g_clusters) + "_manifolds/")
else:
    input_dim = 7
    writer = SummaryWriter("classify/tensorbd/" + object_cls.name + "/pose_" + str(object_cls.g_clusters) + "_manifolds/")
...
if __name__ == "__main__":
    ...
    # Save model
    if position_active:
        torch.save(model, 'classify/trained_models/' + object_cls.name + '/position_' + str(object_cls.g_clusters) +'.pkl')
    else:
        torch.save(model, 'classify/trained_models/' + object_cls.name + '/pose_' + str(object_cls.g_clusters) +'.pkl')
