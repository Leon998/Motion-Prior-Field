...
class NatNetClient:
        ...
        if self.__can_change_bitstream_version and \
            ...
            if return_code >= 0:
                ...
                tmpCommands = [
                    ...
                    "SetPlaybackCurrentFrame,0", "TimelineStop"
                ]
    ...
    def __create_data_socket(self, port):
        ...
        if self.use_multicast:
            ...
            try:
                ...
                print(
                    "ERROR: data socket timeout occurred. Server not responding"
                )
                result = None
        else:
            # Unicast case
    ...
    def __unpack_rigid_body(self, data, major, minor, rb_num):
        ...
        if (major < 3 and major != 0):
            ...
            marker_count = int.from_bytes(data[offset:offset + 4],
                                          byteorder='little')
            offset += 4
            ...
            if major >= 2:
                # Marker ID's
                for i in marker_count_range:
                    new_id = int.from_bytes(data[offset:offset + 4],
                                            byteorder='little')
                    offset += 4
    ...
    def __unpack_skeleton(self, data, major, minor):
        ...
        rigid_body_count = int.from_bytes(data[offset:offset + 4],
                                          byteorder='little')
        offset += 4
...
#Unpack Mocap Data Functions
    ...
    def __unpack_marker_set_data(self, data, packet_size, major, minor):
        ...
        marker_set_count = int.from_bytes(data[offset:offset + 4],
                                          byteorder='little')
        offset += 4
        ...
        for i in range(0, marker_set_count):
            ...
            for j in range(0, marker_count):
                pos = Vector3.unpack(data[offset:offset + 12])
                offset += 12
                trace_mf("\tMarker %3.1d : [%3.2f,%3.2f,%3.2f]" %
                         (j, pos[0], pos[1], pos[2]))
                marker_data.add_pos(pos)
            marker_set_data.add_marker_data(marker_data)

        # Unlabeled markers count (4 bytes)
        unlabeled_markers_count = int.from_bytes(data[offset:offset + 4],
                                                 byteorder='little')
        offset += 4
    ...
    def __unpack_skeleton_data(self, data, packet_size, major, minor):
        ...
        if ((major == 2 and minor > 0) or major > 2):
            skeleton_count = int.from_bytes(data[offset:offset + 4],
                                            byteorder='little')
            offset += 4
    ...
    def __unpack_labeled_marker_data(self, data, packet_size, major, minor):
        ...
        if ((major == 2 and minor > 3) or major > 2):
            labeled_marker_count = int.from_bytes(data[offset:offset + 4],
                                                  byteorder='little')
            offset += 4
            ...
            for _ in range(0, labeled_marker_count):
                model_id = 0
                marker_id = 0
                tmp_id = int.from_bytes(data[offset:offset + 4],
                                        byteorder='little')
                offset += 4
                ...
                size = FloatValue.unpack(data[offset:offset + 4])
                offset += 4
                trace_mf("ID     : [MarkerID: %3.1d] [ModelID: %3.1d]" %
                         (marker_id, model_id))
    ...
    def __unpack_force_plate_data(self, data, packet_size, major, minor):
        ...
        if ((major == 2 and minor >= 9) or major > 2):
            force_plate_count = int.from_bytes(data[offset:offset + 4],
                                               byteorder='little')
            offset += 4
            trace_mf("Force Plate Count:", force_plate_count)
            for i in range(0, force_plate_count):
                # ID
                force_plate_id = int.from_bytes(data[offset:offset + 4],
                                                byteorder='little')
                offset += 4
                force_plate = MoCapData.ForcePlate(force_plate_id)

                # Channel Count
                force_plate_channel_count = int.from_bytes(data[offset:offset +
                                                                4],
                                                           byteorder='little')
                offset += 4
                ...
                for j in range(force_plate_channel_count):
                    ...
                    # Force plate frames
                    n_frames_show = min(force_plate_channel_frame_count,
                                        n_frames_show_max)
    ...
    def __unpack_device_data(self, data, packet_size, major, minor):
        ...
        if (major == 2 and minor >= 11) or (major > 2):
            device_count = int.from_bytes(data[offset:offset + 4],
                                          byteorder='little')
            offset += 4
            ...
            for i in range(0, device_count):
                ...
                # ID
                device_id = int.from_bytes(data[offset:offset + 4],
                                           byteorder='little')
                offset += 4
                device = MoCapData.Device(device_id)
                # Channel Count
                device_channel_count = int.from_bytes(data[offset:offset + 4],
                                                      byteorder='little')
                offset += 4
                ...
                for j in range(0, device_channel_count):
                    ...
                    # Device Frame Data
                    n_frames_show = min(device_channel_frame_count,
                                        n_frames_show_max)
    ...
    def __unpack_frame_suffix_data(self, data, packet_size, major, minor):
        ...
        if major >= 3:
            stamp_camera_mid_exposure = int.from_bytes(data[offset:offset + 8],
                                                       byteorder='little')
            ...
            stamp_data_received = int.from_bytes(data[offset:offset + 8],
                                                 byteorder='little')
            offset += 8
            ...
            stamp_transmit = int.from_bytes(data[offset:offset + 8],
                                            byteorder='little')
            offset += 8
    ...
    def __unpack_rigid_body_description(self, data, major, minor):
        ...
        if (major >= 3) or (major == 0):
            ...
            for marker in marker_count_range:
                ...
                # Active Label
                active_label = int.from_bytes(data[offset2:offset2 + 4],
                                              byteorder='little')
                offset2 += 4
                ...
                rb_marker = DataDescriptions.RBMarker(marker_name,
                                                      active_label,
                                                      marker_offset)
    ...
    def __unpack_force_plate_description(self, data, major, minor):
        ...
        if major >= 3:
            ...
            for i in range(0, 12):
                cal_matrix_row = FPCalMatrixRow.unpack(data[offset:offset +
                                                            (12 * 4)])
                trace_dd(
                    "\t%3.1d %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e %3.3e"
                    % (i, cal_matrix_row[0], cal_matrix_row[1],
                       cal_matrix_row[2], cal_matrix_row[3], cal_matrix_row[4],
                       cal_matrix_row[5], cal_matrix_row[6], cal_matrix_row[7],
                       cal_matrix_row[8], cal_matrix_row[9],
                       cal_matrix_row[10], cal_matrix_row[11]))
            ...
            # Plate Type int
            plate_type = int.from_bytes(data[offset:offset + 4],
                                        byteorder='little')
            offset += 4
            ...
            channel_data_type = int.from_bytes(data[offset:offset + 4],
                                               byteorder='little')
            offset += 4
            ...
            num_channels = int.from_bytes(data[offset:offset + 4],
                                          byteorder='little')
            offset += 4
    ...
    def __unpack_device_description(self, data, major, minor):
        ...
        if major >= 3:
            ...
            # Device Type int
            device_type = int.from_bytes(data[offset:offset + 4],
                                         byteorder='little')
            offset += 4
    ...
    def __unpack_data_descriptions(self, data: bytes, packet_size, major,
                                   minor):
        data_descs = DataDescriptions.DataDescriptions()
        offset = 0
        # # of data sets to process
        dataset_count = int.from_bytes(data[offset:offset + 4],
                                       byteorder='little')
        offset += 4
        trace_dd("Dataset Count : ", str(dataset_count))
        for i in range(0, dataset_count):
            trace_dd("Dataset ", str(i))
            data_type = int.from_bytes(data[offset:offset + 4],
                                       byteorder='little')
            offset += 4
            data_tmp = None
            if data_type == 0:
                ...
                print("\t" + str(i + 1) + " datasets processed of " +
                      str(dataset_count))
                print("\t " + str(offset) + " bytes processed of " +
                      str(packet_size))
                print("\tPACKET DECODE STOPPED")
                return offset
            offset += offset_tmp
            data_descs.add_data(data_tmp)
            trace_dd("\t" + str(i) + " datasets processed of " +
                     str(dataset_count))
            trace_dd("\t " + str(offset) + " bytes processed of " +
                     str(packet_size))
    ...
    def __unpack_server_info(self, data, packet_size, major, minor):
        ...
        if (self.__nat_net_requested_version[0] == 0) and\
           (self.__nat_net_requested_version[1] == 0):
            self.__nat_net_requested_version[
                0] = self.__nat_net_stream_version_server[0]
            self.__nat_net_requested_version[
                1] = self.__nat_net_stream_version_server[1]
            self.__nat_net_requested_version[
                2] = self.__nat_net_stream_version_server[2]
            self.__nat_net_requested_version[
                3] = self.__nat_net_stream_version_server[3]
            # Determine if the bitstream version can be changed
            if (self.__nat_net_stream_version_server[0]
                    >= 4) and (self.use_multicast == False):
                self.__can_change_bitstream_version = True

        trace_mf("Sending Application Name: ", self.__application_name)
        trace_mf("NatNetVersion ",
    ...
    def __command_thread_function(self, in_socket, stop, gprint_level):
        ...
        while not stop():
            ...
            if not self.use_multicast:
                if not stop():
                    self.send_keep_alive(in_socket, self.server_ip_address,
                                         self.command_port)
        return 0
    ...
    def __process_message(self, data: bytes, print_level=0):
        ...
        if message_id == self.NAT_FRAMEOFDATA:
            ...
            trace("Packet Size : ", packet_size)
            offset += self.__unpack_server_info(data[offset:], packet_size,
                                                major, minor)
            ...
            trace("Packet Size : ", packet_size)
            if packet_size == 4:
                command_response = int.from_bytes(data[offset:offset + 4],
                                                  byteorder='little')
                offset += 4
            ...
            trace("Message ID  : %3.1d NAT_UNRECOGNIZED_REQUEST: " %
                  message_id)
            trace("Packet Size : ", packet_size)
    ...
    def send_command(self, command_str):
        ...
        while nTries:
            ...
            ret_val = self.send_request(
                self.command_socket, self.NAT_REQUEST, command_str,
    ...
    def send_keep_alive(self, in_socket, server_ip_address, server_port):
        return self.send_request(in_socket, self.NAT_KEEPALIVE, "",
