...
class TrignoEMG(_BaseTrignoDaq):
    ...
    def read(self, return_read=False):
        """
        Request a sample of data from the device.
        ...
        if self.buffered:
            while self.recordFlag:
                ...
                self.data = data[self.channel_range[0]:self.channel_range[1]+1, :]
                self.buffer = np.hstack((self.buffer, self.scaler * self.data))
        else:
            self.data = super(TrignoEMG, self).read(self.samples_per_read)
            self.data = self.data[self.channel_range[0]:self.channel_range[1]+1, :]
        if return_read:
            return self.scaler * self.data
    

    def flush(self, channel_range):
        self.buffer = np.zeros((channel_range[1] - channel_range[0] + 1,0))
