...
def select_target_obj_pose(predPose, object_names, target_object):
    for p in predPose:
        id, rt, conf, puv, pxyz, opoint, clsid, partid, cx, cy, layerId = p
        # if object_names[int(id)] == target_object:
        if str(target_object) in str(object_names[int(id)]):
            print("Detected target!")
            RT = rt
            detect_flag=1
            break
        else:
            print("No target detection!")
            RT = 0
            detect_flag = 0
    return RT, detect_flag
